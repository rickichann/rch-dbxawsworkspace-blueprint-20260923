...
`company_name = "company"` and `environment = "dev"` you get `company-dbx-dev-vpc`,
`company-dbx-dev-root-bucket`, `company-dbx-dev-catalog`, and so on.
...
| `aws_profile`  | AWS CLI profile for the account        | `company-dev`    |
...
Open the URL and check Catalog Explorer for `{company_name}-dbx-dev-catalog` and its schemas.
...
aws s3 rb s3://company-dbx-dev-terraform-state --force --profile company-dev
```
