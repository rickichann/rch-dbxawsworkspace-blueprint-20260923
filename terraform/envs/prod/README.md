# Databricks on AWS — `prod` environment
...
```bash
bash terraform/envs/prod/create-state-bucket.sh
```
...
```powershell
cd terraform\envs\prod\aws-foundation
terraform init
terraform plan
terraform apply
terraform output
...
```powershell
cd terraform\envs\prod\databricks-workspace
terraform init
terraform plan
terraform apply
terraform output
...
```powershell
cd terraform\envs\prod\databricks-catalog
terraform init
terraform plan
terraform apply
```
...
```powershell
cd terraform\envs\prod\databricks-catalog   ; terraform destroy
